...
export function tdStart(
  transitionPage: transitionPage
): [HTMLDivElement, gsap.core.Timeline] {
  const svg = (
    direction
  ) => `<svg class="dsn-separator-${direction} dsn-icon-assistant-color" width="100%" height="100%" viewBox="0 0 100 10"
  ...
  domTransition.className =
    "dsn-ajax-loader dsn-ajax-normal bg-load background-section d-flex flex-column justify-content-between text-center";
  ...
  const tl = gsap
    ...
    .to(
      domTransition.querySelector(".dsn-separator-top .path-anim"),
      {
...
export function tdEnd(): void {
  document.body.querySelectorAll(".dsn-ajax-loader").forEach((el, index) => {
    ...
    gsap
      ...
      .to(
        el.querySelector(".dsn-separator-bottom .path-anim"),
        {
          attr: { d: "M 0 0 C 40 0 55 0 100 0 L 0 0 Z" },
          overwrite: false,
          duration: 1.5,
