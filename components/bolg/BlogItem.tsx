...
function BlogItem({
  ...
  return (
    <>
      <div
        className={dsnCN(
          "blog-item p-relative d-flex align-items-center h-100 w-100",
          className
        )}
        ref={ref}
        {...parallaxContent}
            ...
            <div className="box-content-body">
              ...
              {portoDetails.title && (
                <h4 className={`title-block mb-10`}>
                  <DsnLink
                    href={`countries/${portoDetails.title}?id=${portoDetails.id}`}
                    transitionPage={{ title: portoDetails.title }}
                  >
                    {portoDetails.title}
                  </DsnLink>
                </h4>
