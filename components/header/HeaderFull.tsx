...
function HeaderFull({
  ...
  useEffect(() => {
    ...
    bg.current &&
      parallax &&
      animate.to(bg.current.children, { ...parallax, ease: "none" }, 0);
    content.current &&
      parallaxContent &&
      animate.to(content.current, { ...parallaxContent, ease: "none" }, 0);
    animateText.current &&
      animate.to(
        animateText.current,
        { clipPath: "inset(-20% 0 -10px 0)", ease: "none" },
        0
      );
  ...
  return (
    <header
      className={dsnCN(
        "header-page overflow",
        (src || video) && !skin && "v-dark-head",
        skin,
        skin && "v-dark-head-mobile",
      ...
      {heroContent && (
        ...
        >
          <div className="content p-relative">
            ...
            {children && (
              <div className="dsn-description mt-30">{children}</div>
            )}
          </div>
        </div>
      )}
