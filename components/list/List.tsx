...
export function ListItem({
  ...
  return (
    <div className={dsnCN("item d-flex align-items-center", className)}>
      ...
      <Tag className={dsnCN(headingTitle, icon && "ml-15")}>{children}</Tag>
    </div>
  );
}
...
function List({
  ...
  children,
  ...restProps
}: { iconSize?: string | number } & DsnGridProps) {
  return (
    <DsnGrid
      className={dsnCN("dsn-list", className)}
      {...restProps}
      style={{ "--dsn-icon-list": iconSize } as CSSProperties}
    >
      {children}
    </DsnGrid>
  );
