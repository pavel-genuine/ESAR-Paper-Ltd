  ...
  const toggleClick = (e) => {
    ...
    if (!reserved) {
      SvgAnimate.up(svg.current, TransEnd)
        .to(
          "#dsn-scrollbar",
          { y: -200, duration: 1, ease: "power4.in" },
          "top"
        )
        .set(backgroundMain.current, { autoAlpha: 1 }, "center");
      onCompleteAnimate(this);
    } else
      SvgAnimate.down(svg.current, () => onCompleteAnimate(this))
        .to(
          "#dsn-scrollbar",
          { y: 0, clearProps: "y", duration: 1, ease: "power4" },
          "-=1"
        )
    ...
    <>
      ...
      >
        ...
        <div className="toggle-text">
          <div className="text-menu" ref={splitMenu}>
            {textMenu}
          </div>
          <div className="text-open" ref={splitOpen}>
            {textOpen}
          </div>
          <div className="text-close" ref={splitClose}>
            {textClose}
          </div>
        </div>
      </div>
